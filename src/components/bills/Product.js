...
export default function Product({
        ...
        products.forEach((prod, index) => {
            if (prod.item._id === item._id) {
               //matched 
                ...
                tempArray[index].select = isNaN(inputQuantity) || inputQuantity === '' ? 1 : parseInt(inputQuantity);
                let totalMrp = total;
                let toSubTract = initialSelect * (item.discounted_mrp ? item.discounted_mrp : item.mrp) //now we have to subtract some amount from out total
                console.log(toSubTract)
                totalMrp = (totalMrp - toSubTract) + ( tempArray[index].select * (item.discounted_mrp ? item.discounted_mrp : item.mrp));
    ...
    function deleteProdFromSelected(item) {
        ...
        setTotal(total => total = total -  (item.select*(item.item.discounted_mrp ? item.item.discounted_mrp : item.item.mrp)))
        setProducts(prods => prods.filter(prod => prod.item._id !== item.item._id));
                    ...
                    <p className='product-info' key={item.item._id}>
                        ...
                        >
                            ...
                            <input type='Number'
                                ...
                                placeholder='1'
                                onChange={e => { onChangeHandler(e, item.item) }}
                            />
                            &nbsp; or
                            <button style={{
                                ...
                                onClick={e => { deleteProdFromSelected(item) }}
                            >
